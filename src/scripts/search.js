...
export default class Search {
  ...
  showMatches(value) {
    ...
    const html = matchArray
      .map((place) => {
        ...
        const newString = String(place)
          .replace('_', ' ')
          .replace('_', ' ');
        // build html with template literal.

        // To highlight the string we have typed
        // in the input box, we use regex.
        const regex = new RegExp(value, 'gi');

        // We replace part of the result with our regex.
        const highLight = newString.replace(regex, `<span class="highlight">${value}</span>`);

        // We return result HTML and pass it the highlighting as well.
  ...
  onLoad() {
    ...
    // time zones into the global array array.
